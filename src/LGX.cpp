...
{
	char *Path = NULL;
	int pathlen = _BuildIOI(NULL, address);
...
WriteDataService_Request *_BuildLgxWriteDataRequest(const char *address, LGX_Data_Type datatype, void *data, int datasize, CIP_UINT number, int *requestsize)
{
	char *Path = NULL;
	int pathlen = _BuildIOI(NULL, address);
...
LGX_Read *_ReadLgxData(Eip_Session *session, Eip_Connection *connection, const char *address, CIP_UINT number)
{
	...
	ReadDataService_Request *request = _BuildLgxReadDataRequest(address, number, &requestsize);
...
int _WriteLgxData(Eip_Session *session, Eip_Connection *connection, const char *address, LGX_Data_Type datatype, void *data, CIP_UINT number)
{
	...
	WriteDataService_Request *request = _BuildLgxWriteDataRequest(address, datatype, data, datasize, number, &requestsize);
	Eip_CDI *DataItem = _ConnectedSend(session, connection, request, requestsize);
	free(request);
	if (DataItem == NULL)
		return Error;
...
int _GetService_ReplyNumber(Eip_Item *dataitem)
{
	...
	case ItemId_UCM:
		rep = (CommonDataService_Reply *)((char*)dataitem + sizeof(Eip_UDI));
		break;
	case ItemId_ConnectedTP:
		rep = (CommonDataService_Reply *)((char*)dataitem + sizeof(Eip_CDI));
		break;
	default:
	...
	case EXECUTE_PCCC+0x80:
		return 1;
	default:
		CIPERROR(Internal_Error, E_UnsupportedService, 0);
		return 0;
	}
}
...
CIP_INT _GetExtendedStatus(CommonDataService_Reply *reply)
{
	...
	if (reply->Status != 0x01FF) {
		return 0;
	} else {
		return (*((CIP_INT*)((char*)&(reply->Status)+2)));
	}
}
...
float _GetLGXValueAsFloat(LGX_Read *reply, int index)
{
	...
	if (index>reply->Varcount-1)
		CIPERROR(Internal_Error, E_OutOfRange, 0);
	if (reply->mask)
		mask = reply->mask;
	switch (reply->type) {
	case DT_BOOL:
		value = (*((BYTE*)(((char*)reply) + sizeof(LGX_Read) + index * reply->elementsize)));
		break;
	case DT_BITARRAY:
		value = (*((CIP_DINT*)(((char*)reply) + sizeof(LGX_Read) + index * reply->elementsize)));
		break;
	case DT_SINT:
		value = (*((CIP_SINT*)(((char*)reply) + sizeof(LGX_Read) + index * reply->elementsize)));
		break;
	case DT_INT:
		value = (*((CIP_INT*)(((char*)reply) + sizeof(LGX_Read) + index * reply->elementsize)));
		break;
	case DT_DINT:
		value = (*((CIP_DINT*)(((char*)reply) + sizeof(LGX_Read) + index * reply->elementsize)));
		break;
	case DT_REAL:
		value = (*((float*)(((char*)reply) + sizeof(LGX_Read) + index * reply->elementsize)));
		break;
	default:
		CIPERROR(Internal_Error, E_UnsupportedDataType, 0);
		return Error;
...
void *_GetData(CommonDataService_Reply *reply)
{
	...
	case LGX_DINT:
	case LGX_REAL:
		return (((char*)reply) + sizeof(CommonDataService_Reply) + sizeof(CIP_INT));
	default:
		CIPERROR(Internal_Error, E_UnsupportedDataType, 0);
		return NULL;
	}
...
int _GetLGXDataSize(LGX_Data_Type DataType)
{
	...
	case LGX_BOOL:
	case LGX_SINT:
		return sizeof(CIP_SINT);
	case LGX_INT:
		return sizeof(CIP_INT);
	case LGX_BITARRAY:
	case LGX_DINT:
	case LGX_REAL:
		return sizeof(CIP_DINT);
	default:
		CIPERROR(Internal_Error, E_UnsupportedDataType, 0);
		return Error;
	}
